...
//   C144 test-program-shape
//   C145 test-program-duration
...
const TEST_PROGRAM_RULES = ['C144', 'C145'];
...
`);
  ...
  it('stays total on a head-less entry (order null — C144 judges the shape)', () => {
    const m = load(`
conformance_test /conf/t {
  name "T"
  preparation {
...
describe('test programs — round-trip (the serializer fixpoint)', () => {
  ...
  it('round-trips a MALFORMED program byte-clean (order-null entries)', () => {
    ...
    // exactly — the linter (C144), not the codec, owns the judgment.
    const malformed = `
conformance_test /conf/t {
  name "T"
  preparation {
...
describe('test programs — lint rules (C144–C145)', () => {
  ...
  it('C144 fires on a preparation step carrying neither action nor drive', () => {
...
`),
    ...
    assert.ok(
      issues.some(
        i =>
          i.check === 'C144' && i.message.includes('neither action nor drive'),
      ),
      `expected the action/drive leg, got: ${issues.map(i => i.message).join('\n')}`,
    );
  });

  it('C144 fires on duplicate and non-positive entry orders', () => {
    const issues = testProgramIssues(
      makeTmpPackage(`
conformance_test /conf/t {
  name "T"
...
`),
    ...
    assert.ok(
      issues.some(
        i => i.check === 'C144' && i.message.includes('declared twice'),
      ),
    ...
    assert.ok(
      issues.some(
        i =>
          i.check === 'C144' && i.message.includes('no positive-integer order'),
      ),
      `expected the non-positive-order leg, got: ${issues.map(i => i.message).join('\n')}`,
    );
  });

  it('C144 fires on a verify block missing read or tolerance', () => {
    const issues = testProgramIssues(
      makeTmpPackage(`
conformance_test /conf/t {
  name "T"
...
`),
    ...
    assert.ok(
      issues.some(
        i => i.check === 'C144' && i.message.includes('without tolerance'),
      ),
      `expected the verify leg, got: ${issues.map(i => i.message).join('\n')}`,
    );
  });

  it('C144 fires on a stimulus point without a drive or an acceptance reference', () => {
    const issues = testProgramIssues(
      makeTmpPackage(`
conformance_test /conf/t {
  name "T"
...
`),
    ...
    assert.ok(
      issues.some(i => i.check === 'C144' && i.message.includes('no drive')),
      `expected the drive leg, got: ${issues.map(i => i.message).join('\n')}`,
    );
    assert.ok(
      issues.some(
        i => i.check === 'C144' && i.message.includes('no acceptance'),
      ),
      `expected the acceptance leg, got: ${issues.map(i => i.message).join('\n')}`,
    );
  });

  it('C145 fires on an unparseable hold', () => {
    const issues = testProgramIssues(
      makeTmpPackage(`
conformance_test /conf/t {
  name "T"
...
`),
    ...
    assert.ok(
      issues.some(
        i =>
          i.check === 'C145' && i.message.includes('not a parseable duration'),
      ),
      `expected the hold leg, got: ${issues.map(i => i.message).join('\n')}`,
    );
  });

  it('C145 fires on a stimulus point without fresh_within, and on an unparseable one', () => {
...
`),
    ...
    assert.ok(
      missing.some(
        i => i.check === 'C145' && i.message.includes('no fresh_within'),
      ),
