...
export const parsePredicate: Parser = function (id, data) {
  ...
  while (i < t.length) {
    ...
    if (i >= t.length) {
      throw new Error(
        `Parsing error: predicate. ID ${id}: Expecting value for ${keyword}`,
      );
    }
    if (keyword === 'kind') {
      ...
      if (v !== 'citation' && v !== 'semantic') {
        throw new Error(
          `Parsing error: predicate. ID ${id}: kind must be citation|semantic (got '${v}')`,
        );
      }
      result.kind = v;
      ...
      result.subjectKinds = tokenizePackage(unwrapBlock(t[i++])).map(
        stripWrapping,
      );
    } else if (keyword === 'target_kinds') {
      result.targetKinds = tokenizePackage(unwrapBlock(t[i++])).map(
        stripWrapping,
      );
  ...
  return ctx => {
    ...
    (ctx as { predicates: Record<string, RefPredicate> }).predicates[id] =
      result;
    return ctx;
  };
};
