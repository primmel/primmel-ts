...
- `kind core|module|rec|product_reference|certification_program` — the package tier (anything else is a parse error; absent means an ordinary rec package); `product_reference` (TODO.roadmap/36, doctrine ch. 15) is a manufacturer's product model mapped aspect-by-aspect to the Recommendation, declaring `manufacturer`/`product`/`maps_to { … }` on the manifest; `certification_program` (smart TODO.v2/01, twin-certification-design Q4) is a scheme operator's certification program — a fourth publisher with the same mapping-only shape (recs via `maps_to { … }`, product twins via pinned abstract imports, composed into nothing), declaring `scheme_type <type-id>` (its ISO/IEC 17067 self-classification; C97/C98 check it);
- `license_key "std:iec-60068-2-30"` + `license_holder "IEC"` — the entitlement facet pair (smart TODO.external-refs/04): the entitlement catalog key the platform's license gate reads, and the content's copyright owner. Optional, and a package without a key is public content (the OIML models stay license-free); a bare holder stays legal (attribution without an entitlement claim). `primmel check` enforces the key shape `^std:[a-z0-9-]+$` (C144), at most one `license_key` per package (C145 — the parser records the overwritten earlier declarations), and the holder required with the key (C146);
...
`primmel export retrieval <package-dir> [--out <file>]` (same `export` arm; primmel-ts#65) projects a package into the canonical retrieval document (`src/export/retrieval.ts`; the contract doc is `docs/retrieval-export.md`): the typed units (requirement / conformance_test / term / attribute / behavior / calculation / formula / symbol / constraint / characteristic / table / sequence / note / state_machine / dimension) with package-authored STABLE PUBLIC IDENTIFIER ids, document-numbered clause provenance on every edge (`{ doc, clause, urn }` — a producer-internal UUID anchor is demoted to the optional `anchor` extra wherever it arrives, and the anchor-only/non-URN debt surfaces in the stats), the distinct `edition` (publication edition) / `model_version` (package version) document fields, a per-unit sha256 `content_hash` over the documented canonical JSON (identity = id, currency = digest) plus the compact machine `passport` an agent can carry and verify, the per-unit flat scalar `facet` (versioned `retrieval-facet/1` on the document's `facet_version`, key-congruent with the consumer's chunk wire schema, applicability flattened to `app_<dim>` keys, a derived projection excluded from the digest input), the language tag + ISO 24229 `variants` per unit (the package's `text` blocks resolved by the C89 longest-prefix address rule onto the kernel element id — authored content, digest-participating; blocks addressed at unprojected elements counted in `stats.droppedTextBlocks`), and the bundle `source_hash` — the same sha256-over-every-package-byte algorithm the deployed consumer's derive-model-plane.ts runs, so its freshness pins key without translation. JSON, byte-deterministic (sorted keys, two-space indent); the projection shape versions as `primmel-retrieval/1`.
